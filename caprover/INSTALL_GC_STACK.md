...
When creating **Resources** for connectors (Twilio message template, GFW API key, `comapeo_server`, `oauth_client_credentials`, etc.), another workspace you already run (e.g. an existing staging instance) is a practical reference for shapes and for org-wide secrets you are allowed to reuse. For **CoMapeo**, the Windmill `comapeo_server` `access_token` should match the archive server’s bearer token configured in CapRover for that deployment (see the CoMapeo app’s environment variables). For **Auth0 metrics**, the Windmill `oauth_client_credentials` resource must use the **GC Metrics** (M2M) application’s Client ID and Client Secret—not the Auth0 application used for Windmill user login; wrong choice yields `403` on the token endpoint and Management API authorization errors. Fields are `client_id`, `client_secret`, and `domain`. See [`gc-scripts-hub` metrics README — Auth0 integration](https://github.com/ConservationMetrics/gc-scripts-hub/blob/main/f/metrics/guardianconnector/README.md#auth0-integration) and [`auth0/README.md`](../auth0/README.md) (M2M step).
