...
def deploy_stack(config, gc_repository, dry_run):
    ...
    if config["postgres"].get("deploy", False):
        ...
        if postgres_vm_port := config["postgres"]["exposePort"]:
            ...
            cap.update_app(app_name, port_mapping=[f"{postgres_vm_port}:{postgres_from_container.port}"])
    ...
    if config.get(one_click_app_name, {}).get("deploy", False):
        ...
        with psycopg.connect(postgres_from_vm.connstr("postgres"), autocommit=True) as conn:
        ...
        if is_using_azure_db and not dry_run:
            # As windmill_login
            postgres_azure_user = replace(postgres_from_vm, user=windmill_db_user, password=windmill_db_pass)
