...
5. Create a M2M application for metrics with a name like **GC Metrics**, and grant `read:users` and `read:stats` scopes to it. This authorizes the metrics job against the Auth0 Management API. For the Windmill `oauth_client_credentials` resource (and other connector resources), follow [**Setting up resources**](../caprover/INSTALL_GC_STACK.md#setting-up-resources) in the stack install guide—do not use the Windmill SSO application’s Client ID/Secret there.
