...
5. Create a M2M application for metrics with a name like **GC Metrics**, and grant `read:users` and `read:stats` scopes to it. This lets the GC Metrics app read statistics from auth0 about other apps. In Windmill, the `oauth_client_credentials` resource for `guardianconnector_metrics` must use **this** M2M app’s Client ID and Secret—not the Auth0 application used for Windmill SSO or other interactive logins (those lack Management API grants and produce `403` / client-grant errors at token time).
